...
router.get('/all', passport.authenticate('jwt', {session:false}), (req,res) => {
    ...
    Transaction.collection.distinct("ticker").then(uniqueTickers => {
        async.forEachOf(uniqueTickers,(ticker, key, callback) =>{
            YahooStocks.lookup(ticker).then(ticketInfo =>{
                currentTickerPrice[ticker] = ticketInfo["currentPrice"];
                callback();
            })
        }, err => {
            if(err) res.status(400).json({error:"Error in getting stock info from yahoo"});
            ...
            Transaction.find().then(transactions => {
                transactions.forEach( transaction => {
                    ...
                    currentTransactionValue["Stock Ticker"] = transaction.ticker;
                    currentTransactionValue["Units"] = transaction.units;
                    currentTransactionValue["Current Price"] = currentTickerPrice[transaction.ticker];
                    currentPortfolio.push(currentTransactionValue);
                    
                });
            res.json(currentPortfolio);
            }).catch(err => res.status(404).json({noPostsFound: "no transactions found"}));
